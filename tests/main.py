...
class Trade:
    ...
    def __init__(self, symbol: str, interval: str) -> None:
        ...
        self.market_url = f'https://www.binance.com/ru/futures/{symbol}'
    ...
    def create_request(candles):
        def decorator(func):
            def wrapper(self):
                url = f'https://fapi.binance.com/fapi/v1/klines?symbol={self.symbol}&interval={self.interval}&limit={candles}'
                r = requests.get(url)
    ...
    def on_message(self, kline: dict) -> None:    # get data from websockets        
        ...
        if not self.is_intersection:
            ...
            logging.info(f'Intersecion stoch => {self.current_position}')
        else:
    ...
    def streaming(self):
        url = f'https://fapi.binance.com/fapi/v1/klines?symbol={self.symbol}&interval={self.interval}&limit=1'
...
if __name__ == '__main__':
    symbol = input('Ввеите монету (Пример: BTCUSDT) => ')
