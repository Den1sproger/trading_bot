...
CANDLES = 299
FILENAME_JSON = '_binance.json'
FILENAME_JSON_REVERSE = '_binance_reverse.json'
...
def parsing_to_json(response: list[list[int, float, str]]) -> None:
    data = {}
    price_types = ['open', 'high', 'low', 'close']    # open price, high price, low price, close price

    for index, key in zip(range(1, 5), price_types):
        data[key] = [float(kline[index]) for kline in response[149:]]

    right_close = data['close']
    left_close = [float(kline[4]) for kline in response[:149]]       # close prices

    right_high = data['high']
    left_high = [float(kline[2]) for kline in response[:149]]         # high prices
...
def get_api_url() -> str:
    while True:
        data = input('Введите тип торговли (Спот/Фьючерсы) s/f => ')
        if data not in ('s', 'S', 'f', 'F'):
...
def main():
    ...
    symbol = input('Ввведите монету (Пример: BTCUSDT) -> ')
    ...
    try:
        url = base_endpoint + f'symbol={symbol.upper()}&interval={interval}&endTime={unix_endtime}999&limit={CANDLES}'
